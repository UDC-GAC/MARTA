...
## Last update Fri Aug 21 10:37:24 2015 Louis-Noel Pouchet
##
...
ALL_RESULTS_FILENAME=all_results_k40.csv;
...
for expe in cusp cusparse; do
    for featSet in base extended1 extended2; do
	for Algo in BFTree SimpleCart; do
	    for minObj in 2 3 4 5; do
		for maxLeaf in 5 10 15; do
			...
			echo "Train...";
			java -cp $WEKA_ABS_JAR_PATH  weka.classifiers.trees.$Algo -M $minObj -N $maxLeaf -t $trainfile.arff -d output_model.model > model_learn_stats.txt;
			echo "Produce classified outputs...";
			java -cp $WEKA_ABS_JAR_PATH weka.classifiers.trees.$Algo -l output_model.model -T $testfile.arff -p 0 > pred_$foldername.csv;
			echo "Produce tester stats...";
			java -cp $WEKA_ABS_JAR_PATH weka.classifiers.trees.$Algo -l output_model.model -T $testfile.arff  > model_test_stats.csv;

			## Prepare results.
			fulldb="full";
			dbfile=$rootdir/$DIR_PATH_TO_ARFF_FILES/$expe$usc$fulldb$usc$i$usc$test.arff;
			if ! [ -f "$rootdir/$DIR_PATH_TO_ARFF_FILES/$expe$usc$fulldb$usc$i$usc$test.arff" ]; then exit 42; fi;
			rm -f "train_stats_summary_gfs_$foldername.csv";
			rm -f "gfs_stats_$foldername.csv";
			compute_metrics "pred_$foldername.csv" "$dbfile" "$expe" "gfs_stats_$foldername.csv" "$rootdir/$ALL_RESULTS_FILENAME" "$expe $featSet $Algo $minObj $maxLeaf $i";
			cd $rootdir;
		    done;
		done;
	    done;
